...
class WarframeRivenWeaponMapper:
    ...
    async def _fetch_lich_weapons(self) -> list[RivenWeapon]:
        url = f"{WARFRAME_MARKET_V2_BASE_URL}/lich/weapons"
        headers = {
    ...
    async def refresh_cache(self) -> int:
        weapons = await self._fetch_lich_weapons()
        if not weapons:
            return 0
